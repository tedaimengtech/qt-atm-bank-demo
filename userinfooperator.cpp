    ...
    , mode(mode)
{
    ...
    //设置输入限制
    //数字限制
    ui->account->setValidator(new QRegularExpressionValidator(QRegularExpression(NUMBER_REGEXP)));
    ui->password->setValidator(new QRegularExpressionValidator(QRegularExpression(NUMBER_REGEXP)));
    ui->balance->setValidator(new QRegularExpressionValidator(QRegularExpression(AMOUNT_REGEXP)));
    ui->maxDailyTransfer->setValidator(new QRegularExpressionValidator(QRegularExpression(AMOUNT_REGEXP)));
    ui->maxSingleTransfer->setValidator(new QRegularExpressionValidator(QRegularExpression(AMOUNT_REGEXP)));
    //身份证号限制
    ui->id->setValidator(new QRegularExpressionValidator(QRegularExpression(ID_REGEXP)));
    //姓名限制
    ui->name->setValidator(new QRegularExpressionValidator(QRegularExpression(NAME_REGEXP)));
...
bool UserInfoOperator::inputValidCheck() {
    ...
    if (balance % 100 != 0 && mode == userMode::createUser) {
...
void UserInfoOperator::on_buttonBox_accepted() {
    ...
    if (!inputValidCheck() || ui->name->text().isEmpty()
        ...
        || ui->maxDailyTransfer->text().isEmpty()
        || ui->maxSingleTransfer->text().isEmpty()
    ) {
    ...
    DataOperator::userInfo newUser {
        ...
        .name = ui->name->text(),
        .id = ui->id->text(),
        .balance = DataOperator::balanceStringToNumber(ui->balance->text()),
        .maxSingleTransferAmount = DataOperator::balanceStringToNumber(ui->maxSingleTransfer->text()),
        .maxDailyTransferAmount = DataOperator::balanceStringToNumber(ui->maxDailyTransfer->text())
    };
    //写入数据库
